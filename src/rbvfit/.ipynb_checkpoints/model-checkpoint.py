...
def map_theta2list(theta,ncl=1,nt=1):
    ...
    for i in range(0,nt):  
        N_prime[cl*i:cl*i+cl]=N[0:ncl] 
        b_prime[cl*i:cl*i+cl]=b[0:ncl]
        v_prime[cl*i:cl*i+cl]=vel[0:ncl]
...
class create_voigt(object):
    ...
    def __init__(self, zabs,lambda_rest,nclump,ntransition, FWHM = '6.5', grating='G130M',life_position='1'):
        ...
        for i in range(0,ntransition):
            lam_restlist=np.append(lam_restlist,np.repeat(lam_rest[i],nclump))
            lam_restlist=np.append(lam_restlist,lam_rest[ntransition:])
