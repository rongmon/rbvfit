...
def load_spectrum(slice_name):
    ...
    flux, error, wave = u.clean_flux_error_wave(s_HIRES.fnorm, s_HIRES.enorm, s_HIRES.wave_slice)
...
# Select random sample of posterior iterations for uncertainty visualization
n_samples = min(300, len(samples))  # Use up to 300 iterations
sample_indices = np.random.choice(len(samples), size=n_samples, replace=False)

print(f"Plotting {n_samples} posterior samples to show parameter uncertainty")

# Create two-panel plot for both instruments
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))

# ============================================================================
# PANEL 1: XShooter Data and Model
# ============================================================================
print("Creating XShooter panel...")

# Plot observed data
ax1.step(wave, flux, 'k-', where='mid', linewidth=1, alpha=0.8, label='XShooter Data')
ax1.step(wave, error, 'gray', where='mid', alpha=0.3, linewidth=0.5, label='1σ Error')

# Plot posterior uncertainty cloud (300 random model iterations)
print("  Adding posterior uncertainty cloud...")
for i in sample_indices:
    model_flux_sample = model_xshooter(samples[i], wave)
    ax1.plot(wave, model_flux_sample, 'blue', alpha=0.01, linewidth=0.5)

# Plot best-fit model
print("  Adding best-fit model...")
best_model_xshooter = model_xshooter(best_theta, wave)
ax1.plot(wave, best_model_xshooter, 'red', linewidth=2, label='Best Fit')

# Mark component velocity locations
print("  Marking component velocities...")
z_abs = 0.0  # Absorption redshift
lambda_rest = 1302.17  # OI rest wavelength
for i in range(len(best_theta)//3):  # Number of components
    v_comp = best_theta[2*len(best_theta)//3 + i]  # Extract velocity parameter
    # Convert velocity to observed wavelength
    lambda_obs = lambda_rest * (1 + z_abs) * (1 + v_comp/299792.458)
    ax1.axvline(lambda_obs, color='orange', linestyle='--', alpha=0.7)
    ax1.text(lambda_obs, 1.1, f'v={v_comp:.0f}', rotation=90, ha='center')

# Format XShooter panel
ax1.set_ylabel('Normalized Flux')
ax1.set_title('XShooter: OI 1302 (FWHM=2.2 pixels, high resolution)')
ax1.legend()
ax1.grid(True, alpha=0.3)
ax1.set_ylim(0, 1.2)

# ============================================================================
# PANEL 2: FIRE Data and Model  
# ============================================================================
print("Creating FIRE panel...")

# Plot observed data
ax2.step(wave1, flux1, 'k-', where='mid', linewidth=1, alpha=0.8, label='FIRE Data')
ax2.step(wave1, error1, 'gray', where='mid', alpha=0.3, linewidth=0.5, label='1σ Error')

# Plot posterior uncertainty cloud (same 300 iterations)
print("  Adding posterior uncertainty cloud...")
for i in sample_indices:
    model_flux_sample = model_fire(samples[i], wave1)
    ax2.plot(wave1, model_flux_sample, 'blue', alpha=0.01, linewidth=0.5)

# Plot best-fit model
print("  Adding best-fit model...")
best_model_fire = model_fire(best_theta, wave1)
ax2.plot(wave1, best_model_fire, 'red', linewidth=2, label='Best Fit')

# Mark component velocities (same as XShooter since parameters are shared)
print("  Marking component velocities...")
for i in range(len(best_theta)//3):
    v_comp = best_theta[2*len(best_theta)//3 + i]
    lambda_obs = lambda_rest * (1 + z_abs) * (1 + v_comp/299792.458)
    ax2.axvline(lambda_obs, color='orange', linestyle='--', alpha=0.7)

# Format FIRE panel
ax2.set_xlabel('Wavelength (Å)')
ax2.set_ylabel('Normalized Flux')
ax2.set_title('FIRE: OI 1302 (FWHM=4.0 pixels, lower resolution)')
ax2.legend()
...
plt.tight_layout()
plt.suptitle(f'Joint Multi-Instrument Fit: OI 1302\n' +
             f'Best-fit: N={best_theta[0]:.2f}, b={best_theta[1]:.1f} km/s, v={best_theta[2]:.1f} km/s',
             y=0.98, fontsize=12)
